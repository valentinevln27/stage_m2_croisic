...
def add_interpolated_points(start, end, step_initial, max_dist, factor, reverse_id=False):
    ...
    -------
    list
        A list of interpolated Point objects from start to end, spaced increasingly 
        up to `max_dist`, by a factor f and then uniformly if the remaining 
...
def add_fixed_spacing_points(start, end, step):
    ...
    start : tuple of float
        ...
        The ending point as (x, y) coordinates.
    step : float
        Desired distance between each point (in the same unit as the coordinates:
        meters).

    Returns
    -------
    points : list of shapely.geometry.Point
        A list of Point objects placed at regular intervals along the line from 
        start to end. The end point itself is not included unless it coincides 
        with the last step.
    """
    points = [] # List to stock all points coordinates and corresponding id
    # Extract coordinates from the start and end points
...
northern_boundary = add_interpolated_points(coast_last, NW_point, 500, 5000, 1.3)
western_boundary = add_fixed_spacing_points(NW_point, SW_point, 5000)
southern_boundary = add_interpolated_points(coast_first, SW_point, 500, 5000, 1.3, 
                                            reverse_id=True)  
