...
ks = 1 # The Strickler coefficient or combination number of the selected simulation
# Lists to store the results of the error metrics
ver = '' # '' if v1 or '_vi' if version i
MAE = [] 
RMSE = []
PBIAIS = [] 
for i in range(1, 7):
    ...
    file_path_pt = f'../bluekenue_stage/point/combi_point{ver}/ks{ks}{ver}/point_{i}_ks{ks}{ver}.ts1'
...
file_path_pt = f'../bluekenue_stage/point/combi_point{ver}/Ks{ks}{ver}/point_1_ks{ks}{ver}.ts1'
df_pt = pd.read_csv(file_path_pt, sep=r'\s+', header=24, names=['sealevel'])
df_pt['time'] = pd.date_range(start='2023-12-30',periods=len(df_pt), freq='5min')
# Defining the time as the index
df_pt.set_index('time', inplace=True)
