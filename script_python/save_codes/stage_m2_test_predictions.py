# -*- coding: utf-8 -*-
"""
Created on Fri Apr 25 13:16:12 2025
...
p_storm['t_in'] = pd.to_datetime(p_storm['t_in'], dayfirst=True)
p_storm.set_index('t_in', inplace=True)

# Extracting the data of a point from the simulation
file_path_so = r'C:/Users/vanle/Downloads/sables_tides_prediction_martin.dat'
df_so = pd.read_csv(file_path_so, delimiter=';', header=1, names=['time', 'sealevel'])
df_so['time'] = pd.to_datetime(df_so['time'], dayfirst=True)
df_so.set_index('time', inplace=True)

# Merge simulation with storm tide prediction
df_merged = pd.merge(p_storm, df_so, left_index=True, right_index=True, how='inner')
...
plt.plot(df_merged.index, df_merged.sealevel, color='red', linestyle='-', linewidth=0.5,
         label='Prediction Le Croisic')
plt.plot(df_merged.index, df_merged.h , color='royalblue', 
         linestyle='-', linewidth=0.5,
         label="Prediction Sables d'Olonne")

# Adding legend
plt.title(f'Predicted tide at Le Croisic', fontsize=20, 
          fontweight='bold')
