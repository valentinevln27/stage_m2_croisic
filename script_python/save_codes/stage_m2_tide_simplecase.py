# -*- coding: utf-8 -*-
"""
Created on Thu Mar  6 15:18:30 2025
...
df_tg = pd.read_csv(file_path_tg, delimiter=';', header=1, names=['time', 'z'])
...
df_tg.loc[df_tg['z'] > 7, 'z'] = np.nan
df_tg['z'] = df_tg['z'] 
df_tg = df_tg.resample('10T').first()  # Keep the first value in each 10-minute interval

# Calculating the predicted tide with Utide
cst_tg = solve(df_tg.index, df_tg.z, lat=47.2980601, trend=False) # Latitude provided by the SHOM
...
for i in range(1, 7):
    ...
    # file_path_pt = f'../bluekenue_stage/point/compa_point/Ks{ks}/point_{i}_tg_vs_model_ks{ks}.ts1'
    file_path_pt = f'../bluekenue_stage/point/simplecase_point/simplecase_point_{i}.ts1'
    df_pt = pd.read_csv(file_path_pt, sep=r'\s+', header=24, names=['sealevel'])
    df_pt['time'] = pd.date_range(start='2023-12-30',periods=len(df_pt), freq='10min')
    # Defining the time as the index
    ...
    maev, maev_un = mae(df_merged['h'], df_merged['sealevel']+0.1)
    rmsev, rmsev_un = rmse(df_merged['h'], df_merged['sealevel']+0.1)
    p_biais = pbiais(df_merged['h'], df_merged['sealevel']+0.1)
    # Storing the results
    MAE.append(maev)
    RMSE.append(rmsev)
...
print('')
print(f'{min(MAE):.2f}; {max(MAE):.2f}')
print(f'{min(RMSE):.2f}; {max(RMSE):.2f}')
print(f'{min(PBIAIS):.2f}%; {max(PBIAIS):.2f}%')    
closest_to_zero = min(PBIAIS, key=abs)
print(f'{closest_to_zero:.2f}%')

#%% Graphical representation with one point only
file_path_pt = '../bluekenue_stage/point/simplecase_point/simplecase_point_1.ts1'
...
plt.plot(df_pt.index, df_pt.sealevel, color='red', linestyle='-', linewidth=0.5,
         label='Model')
plt.plot(p_tg.t_in, p_tg.h , color='royalblue', 
