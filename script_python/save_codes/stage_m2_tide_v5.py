...
tg = ['99', '37', '62'] # In order, Le Croisic, Saint-Nazaire and les Sables d'Olonne
file_tg_metadata = '../datas_stage_m2/gauges_hf_vtd_vh_ZH.txt'

# file_path = f'../datas_stage_m2/records/outputs/{storm}/{tg}_{storm}.txt'
# df_tg = pd.read_excel(file_path)
# df_tg.time = pd.to_datetime(df_tg.time)
# df_tg['time'] = pd.to_datetime(df_tg['time']).dt.round('S')

# date_limite = pd.to_datetime('2000-01-01 23:00:00')
# data_filtered = df_tg[df_tg['time'] <= date_limite]
# plt.plot(data_filtered.time, data_filtered.le_croisic)
# data_filtered.le_croisic = data_filtered.le_croisic


# # Calculating the predicted tide with Utide
# cst_tg = solve(data_filtered.time, data_filtered.le_croisic, lat=47.3, trend=None) # Latitude provided by the SHOM
# predicted_tg = reconstruct(data_filtered.time, cst_tg) # Calculate the predicted tide with utide
...
PBIAIS = [] 
for i in range(1, 7):
    file_path = f'C:/Users/vanle/Downloads/node_31674.csv'
    df_pt = pd.read_csv(file_path, header=2, 
    ...
    df_merged = pd.merge(data_filtered, df_pt, left_on='time', right_index=True, how='inner')
    # Computing MAE, RMSE, and Percentage Bias
    maev, maev_un = mae(df_merged['le_croisic'], df_merged['sealevel'])
    rmsev, rmsev_un = rmse(df_merged['le_croisic'], df_merged['sealevel'])
    p_biais = pbiais(df_merged['le_croisic'], df_merged['sealevel'])
    # Storing the results
    MAE.append(maev)
    RMSE.append(rmsev)
...
plt.plot(df_merged.index, df_merged.sealevel, color='red', linestyle='-', linewidth=0.5,
         label='Model')
plt.plot(df_merged.index, df_merged.le_croisic , color='royalblue', 
