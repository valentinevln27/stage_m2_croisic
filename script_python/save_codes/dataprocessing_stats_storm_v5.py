# -*- coding: utf-8 -*-
"""
Created on Tue May 13 13:24:47 2025
...
ver = '2' 
# Parameters data
base_path = f'../datas_stage_m2/data_weather_points{ver}/'
files = {
...
if ver == '2' :
    df_patm, df_u10, df_v10 = [load_data2(base_path + fname) for fname in files.values()]
else:
    df_patm, df_u10, df_v10 = [load_data(base_path + fname) for fname in files.values()]

base_path = f'../datas_stage_m2/data_swell_points{ver}/'
files = {
...
if ver == '2' :
    df_hs, df_dp, df_tp = [load_data2(base_path + fname) for fname in files.values()]
else:
...
for i in range(df_hs.shape[1]):
    # Compute the 95th percentile of Hs
    percentile_95 = df_hs.iloc[:,i].quantile(0.95)
    # Store the threshold
...
for i in range(df_hs.shape[1]):
    ...
    extremes_Hs = get_extremes(df_hs.iloc[:,i], "POT", threshold=selected_Hs, r="24h")
    # Create windows around those wave height extremes using surrounding minima
    storm_windows1 = create_minimas_windows(df_hs.iloc[:,i], extremes_Hs, selected_Hs)

    # Identify low-pressure events (below 980 hPa)    
    low_pressure_events = df_patm[df_patm.iloc[:,i] < 98000].iloc[:,i]
    low_pressure_events = pd.DataFrame({
        'Patm': low_pressure_events
        }, index=pd.to_datetime(low_pressure_events.index))
    # Create windows around low-pressure events using surrounding maxima
    storm_windows2 = create_maximas_windows(df_patm.iloc[:,i], low_pressure_events, 98000)
    
    # Combine wave-based and pressure-based storm windows
    storm_windows = pd.concat([storm_windows1, storm_windows2], ignore_index=True)
    storm_windows = storm_windows.drop_duplicates()
    storm_windows = storm_windows.sort_values(by='start').reset_index(drop=True)
...
for i in range(df_hs.shape[1]):
    ...
    dfs_param = pd.DataFrame({
        ...
        'Dp': df_dp.iloc[:, i],
        'Tp': df_tp.iloc[:, i]
        }, index=pd.to_datetime(df_hs.index))
    ...
    dfw_param = pd.DataFrame({
        ...
        'u10': df_u10.iloc[:, i],
        'v10': df_v10.iloc[:, i]
        }, index=pd.to_datetime(df_patm.index))
    
    # Retrieve storm time windows detected at this point
    storm_windows = common_storms_windows[i-1]
    ...
    for start, end in storm_windows.values:
        ...
        # and period  
        geo_dp = (450 - params_filtered['Dp']) % 360
        records_s.append({
            'start': start,
            'end': end,
            'Average Hs (m)': params_filtered['Hs'].mean(),
            'Average Dp (°)': geo_dp.mean(),
            'Average Tp (s)': params_filtered['Tp'].mean()})
        try:
            ...
            records_w.append({
                'start': start,
                'end': end,
                'Minimal Patm (Pa)': paramw_filtered['patm'].min(),
...
for i, df in enumerate(merged_all_points):  
    for j, unique_df in enumerate(unique_dfs):
        if df.equals(unique_df):
            grouped_ids[j].append(df_hs.columns[i])  
            break
...
for i, merged_point in enumerate(unique_dfs):
    ...
    node_id = min(grouped_ids[i])
    
    # Legend
    fig.suptitle(f'Swell and weather parameters at {node_id}',
...
for i, param in enumerate(all_params):
    ...
    counts, bins, _ = ax.hist(data_param[param], bins=18, color='mediumaquamarine',
                              edgecolor='seagreen')
    ...
    if param == 'Minimal Patm (Pa)':
        storm_980 = data_param[data_param[param] < 98000]
        print(f'Number of strom with Patm < 980 hPa: {len(storm_980)}')
        storm_1010 = data_param[data_param[param] > 101000]
        print(f'Number of strom with Patm > 1010 hPa: {len(storm_1010)}')
...
remarkable_storm['end'] = pd.to_datetime(remarkable_storm['end'])
...
for i, row in remarkable_storm.iterrows():
    r_start = row['start']
    r_end = row['end']
...
# Calculating the percentage of retrieved storms
p_1 = df_matched.shape[0]/remarkable_storm.shape[0]*100
print(f'Percentage of retrieved storms : {p_1:.2f}%')

# Limit date of storms detected only by Patm
cutoff_date = pd.Timestamp("2023-04-30")
# Remarkable storms after the limit date
remarkable_post_cutoff = remarkable_storm[remarkable_storm['start'] > cutoff_date]
# Retrieved storms after the limit date
matched_post_cutoff = df_matched[df_matched['start'] > cutoff_date]
# Number of storm who weren't retrieved after the limit date
not_found_post_cutoff = remarkable_post_cutoff.shape[0] - matched_post_cutoff.shape[0]
print(f"\nNumber of remarkable storms after April 30, 2023 : {not_found_post_cutoff}")
p_2 = df_matched.shape[0]/(remarkable_storm.shape[0]-not_found_post_cutoff)*100
print(f'Percentage of retrieved storms with Hs and Patm : {p_2:.2f}%')
